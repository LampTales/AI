...
class ProblemSolvingAgent:
    ...
    def GBFS(self, obstacles, start_pos, goal_pos):
        ...
        heap.put((self.euclidean_distance(start_pos, goal_pos), start_pos))
        while heap:
            if self.gbfs(obstacles, heap, visited, map, goal_pos):
                break
        path = self.parents2path(map, goal_pos, start_pos)
        return path, visited

    def gbfs(self, obstacles, heap, visited, map, goal_pos):
        x = heap.get()
        if x[1] == goal_pos:
            return True
        for i in self.neighbours_of(obstacles, x[1]):
            if i[0] not in visited:
                visited.append(i[0])
                heap.put((self.euclidean_distance(i[0], goal_pos), i[0]))
                map[i[0]] = x[1]
        return False
    ...
    def Astar(self, obstacles, start_pos, goal_pos):
        ...
        map = {}
        heap = queue.PriorityQueue()
        heap.put((self.euclidean_distance(start_pos, goal_pos), start_pos, 0))
        while heap:
            if self.astar(obstacles, heap, visited, map, goal_pos):
                break
        path = self.parents2path(map, goal_pos, start_pos)
        return path, visited

    def astar(self, obstacles, heap, visited, map, goal_pos):
        x = heap.get()
        if x[1] == goal_pos:
            return True
        for i in self.neighbours_of(obstacles, x[1]):
            if i[0] not in visited:
                visited.append(i[0])
                heap.put((self.euclidean_distance(i[0], goal_pos) + self.euclidean_distance(i[0], x[1]) + x[2], i[0], self.euclidean_distance(i[0], x[1]) + x[2]))
                map[i[0]] = x[1]
        return False
